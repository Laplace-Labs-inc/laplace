...
// [LAPLACE PATCH] parking_lot::Mutex → TrackedStdMutex wrapper
#[cfg(feature = "laplace")]
mod mutex_wrapper {
    ...
    pub struct MutexGuard<'a, T> {
        inner: laplace_probe_sdk::TrackedStdGuard<'a, T>,
    }
    ...
    impl<T> Mutex<T> {
        pub fn new(value: T) -> Self {
            Self {
                inner: TrackedStdMutex::new(value, "r2d2_pool_internals"),
            }
        }
...
#[cfg(feature = "laplace")]
use mutex_wrapper::{Mutex, MutexGuard};
    ...
    M: ManageConnection,
{
    ...
    fn wait_for_initialization(&self) -> Result<(), Error> {
        ...
        while internals.num_conns != initial_size {
            #[cfg(feature = "laplace")]
            {
                // laplace mode: spin-wait instead of Condvar.wait_until()
                drop(internals);
