...
pub(crate) mod sync {
    ...
    // [LAPLACE PATCH] — Ki-DPOR instrumented Mutex
    #[cfg(feature = "laplace")]
    #[allow(unused_imports)]
    pub(crate) use crate::loom::std::laplace_mutex::{Mutex, MutexGuard};
    ...
    #[cfg(all(feature = "parking_lot", not(miri), not(feature = "laplace")))]
    #[allow(unused_imports)]
    pub(crate) use crate::loom::std::parking_lot::{
        Condvar, Mutex, MutexGuard, RwLock, RwLockReadGuard, WaitTimeoutResult,
    };

    #[cfg(all(not(all(feature = "parking_lot", not(miri))), not(feature = "laplace")))]
    #[allow(unused_imports)]
