...
// [LAPLACE PATCH] — 내부 waiter 큐 계측
// RUSTSEC-2020-0059/0062와 다른 영역 타겟: stale waiter starvation 경로
#[cfg(feature = "laplace")]
use laplace_probe_sdk::{TrackedStdGuard, TrackedStdMutex};

#[cfg(feature = "laplace")]
type WaiterMutex = TrackedStdMutex<Slab<Waiter>>;
#[cfg(not(feature = "laplace"))]
type WaiterMutex = StdMutex<Slab<Waiter>>;
#[cfg(feature = "laplace")]
type WaiterGuard<'a> = TrackedStdGuard<'a, Slab<Waiter>>;
#[cfg(not(feature = "laplace"))]
type WaiterGuard<'a> = std::sync::MutexGuard<'a, Slab<Waiter>>;
...
impl<T> Mutex<T> {
    ...
    pub fn new(t: T) -> Self {
        Self {
            state: AtomicUsize::new(0),
            waiters: TrackedStdMutex::new(Slab::new(), "futures_mutex_waiter_slab"),
            value: UnsafeCell::new(t),
        }
    }
...
impl<T: ?Sized> Mutex<T> {
    #[inline]
    fn lock_waiters(&self) -> WaiterGuard<'_> {
        #[cfg(feature = "laplace")]
        {
            self.waiters.lock()
        }
