...
// [LAPLACE PATCH] Conditional Mutex selection for Ki-DPOR verification
#[cfg(feature = "laplace")]
mod mutex_wrapper {
    ...
    impl<T> Mutex<T> {
        pub fn new(value: T) -> Self {
            Self {
                inner: TrackedMutex::new(value, "pool_internals"),
            }
        }

        pub async fn lock(&self) -> laplace_probe_sdk::TrackedGuard<'_, T> {
...
#[cfg(feature = "laplace")]
use mutex_wrapper::Mutex;
#[cfg(feature = "laplace")]
use laplace_probe_sdk::TrackedGuard as MutexGuard;
