...
//! Differential fidelity gate for the `laplace_rt::watch` shadow seam (AXM2
...
//! `tokio::sync::watch` (column A) and `laplace_rt::watch` (column B, no
...
async fn w1_initial_value_observed_via_borrow_matches() {
    ...
    scenario!(laplace_rt::watch::channel(1u8));
}
...
async fn w2_send_then_changed_ok_then_borrow_and_update_matches() {
    ...
    scenario!(laplace_rt::watch::channel(0u8));
}
...
async fn w7_event_stream_matches_expected_sequence() {
    ...
    {
        let (tx, mut rx1) = laplace_rt::watch::channel::<u8>(0);
        let rx2 = tx.subscribe();
...
async fn w8_send_parity_with_raw_tokio() {
    ...
    require_send::<laplace_rt::watch::Sender<u8>>();
    require_send::<laplace_rt::watch::Receiver<u8>>();
    require_send::<laplace_rt::watch::ModelWatchChanged<'static>>();

    let (tx, mut rx) = laplace_rt::watch::channel::<u8>(0);
