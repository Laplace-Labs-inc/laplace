...
use crate::ids::{ResourceId, ThreadId};
use serde::{Deserialize, Serialize};

/// DPOR-compatible synchronization operation.
///
/// The variants intentionally mirror the seven `laplace-dpor::Operation`
/// variants. They live in this contract crate, not in the DPOR implementation
/// crate, so that an operation source can be written against the contract alone
/// — a source that had to name the DPOR core in order to describe an operation
/// would drag the whole engine into every consumer.
#[repr(u8)]
