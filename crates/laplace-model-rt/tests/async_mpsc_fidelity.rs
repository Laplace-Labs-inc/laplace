...
//! Differential fidelity gate for the `laplace_model_rt::mpsc` shadow seam (AXM2
...
//! `tokio::sync::mpsc` (column A) and `laplace_model_rt::mpsc` (column B, no hook
...
async fn m1_uncontended_send_then_recv_matches() {
    ...
    scenario!(laplace_model_rt::mpsc::channel::<u8>(4));
}
...
async fn m2_backpressure_send_queues_then_recv_unblocks_matches() {
    ...
    scenario!(laplace_model_rt::mpsc::channel::<u8>(1));
}
...
async fn m6_unbounded_send_immediate_then_recv_matches() {
    ...
    scenario!(laplace_model_rt::mpsc::unbounded_channel::<u8>());
}
...
async fn m8_event_stream_matches_expected_sequence() {
    ...
    {
        let (tx, mut rx) = laplace_model_rt::mpsc::channel::<u8>(1);
...
async fn m9_send_parity_with_raw_tokio() {
    ...
    require_send::<laplace_model_rt::mpsc::Sender<u8>>();
    require_send::<laplace_model_rt::mpsc::Receiver<u8>>();
    require_send::<laplace_model_rt::mpsc::ModelMpscSend<'static, u8>>();
    require_send::<laplace_model_rt::mpsc::ModelMpscRecv<'static, u8>>();
