...
//! shadow (`laplace_model_rt::time`).
//!
//! T1-T5 run the *same* assertions against raw `tokio::time` (column A) and
//! `laplace_model_rt::time` (column B, no [`AsyncTimerHook`] installed =
...
use laplace_model_rt::{clear_async_timer_hook, install_async_timer_hook, AsyncTimerHook};
...
/// state (mirrors `laplace-model-rt`'s own `TEST_GUARD` convention).
...
async fn t1_sleep_completion_order_matches() {
    ...
    let short_b = laplace_model_rt::time::sleep(Duration::from_millis(10));
    let long_b = laplace_model_rt::time::sleep(Duration::from_millis(50));
    tokio::pin!(short_b);
    tokio::pin!(long_b);
...
async fn t2_timeout_value_first_and_deadline_first_matches() {
    ...
    let ok_b = laplace_model_rt::time::timeout(Duration::ZERO, async { 42_u8 }).await;
    ...
    let mut err_b = Box::pin(laplace_model_rt::time::timeout(
        Duration::from_millis(10),
        std::future::pending::<()>(),
    ));
...
async fn t3_interval_tick_spacing_matches() {
    ...
    let mut b = laplace_model_rt::time::interval(Duration::from_millis(10));
...
fn t5_send_parity_with_raw_tokio() {
    ...
    require_send::<laplace_model_rt::time::Sleep>();

    require_send::<tokio::time::Timeout<std::future::Ready<()>>>();
    require_send::<laplace_model_rt::time::Timeout<std::future::Ready<()>>>();

    require_send::<tokio::time::Interval>();
    require_send::<laplace_model_rt::time::Interval>();
}
...
fn h1_hooked_sleep_resolves_after_hook_advances_now() {
    let _serial = serial();
    laplace_model_rt::reset_model_async_ids_for_model();
    let hook = std::sync::Arc::new(FakeTimerHook::new(0));
    install_async_timer_hook(hook.clone());

    let mut fut = Box::pin(laplace_model_rt::time::sleep(Duration::from_nanos(100)));
    assert!(
...
fn h2_hooked_sleep_already_elapsed_resolves_without_register() {
    ...
    let mut fut = Box::pin(laplace_model_rt::time::sleep(Duration::ZERO));
