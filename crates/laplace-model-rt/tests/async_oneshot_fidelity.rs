...
//! Differential fidelity gate for the `laplace_model_rt::oneshot` shadow seam
...
//! `tokio::sync::oneshot` (column A) and `laplace_model_rt::oneshot` (column B, no
...
async fn o1_send_then_await_ok_matches() {
    ...
    scenario!(laplace_model_rt::oneshot::channel::<u8>());
}
...
/// [`laplace_model_rt::oneshot`]'s honesty contract: the receive op id is
...
async fn o5_event_stream_matches_expected_sequence() {
    ...
    {
        let (tx, mut rx) = laplace_model_rt::oneshot::channel::<u8>();
...
async fn o6_send_parity_with_raw_tokio() {
    ...
    require_send::<laplace_model_rt::oneshot::Sender<u8>>();
    require_send::<laplace_model_rt::oneshot::Receiver<u8>>();

    let (tx, rx) = laplace_model_rt::oneshot::channel::<u8>();
