...
// `ThreadId` and `ResourceId` are defined by the Axiom execution contract, not
// by this module — `ExecutionSource::step` takes a `ThreadId` and an
// `AxiomOperation` is meaningless without a `ResourceId`, so they cannot be
// stated separately from it (LEP-0035 §4.2). They are re-exported here so that
// `laplace_interfaces::domain::{ThreadId, ResourceId}` keeps resolving; the
// re-export goes away with the rest of the v1 surface in slice ⑤.
pub use laplace_axiom_contract::{ResourceId, ThreadId};
